    ...
    <div className={`rounded-xl border p-3 ${
      ...
      {hasFight ? (
        <div className="mb-2">
          ...
          <div className="flex justify-between mt-1">
            <span className="text-[10px] text-emerald-400">{bullTeams.length} bull</span>
            <span className="text-[10px] text-red-400">{bearTeams.length} bear</span>
          </div>
        </div>
      ) : (
        <div className="mb-2">
      ...
      {polarization > 0.5 && (
        <div className="flex items-center gap-1 mt-1">
          ...
          <span className="text-[10px] text-red-400 font-medium">{Math.round(polarization * 100)}% split</span>
        </div>
      )}
    </div>
  );
...
export default function CycleCard({ cycle, events, defaultOpen }: {
  cycle: CycleData;
