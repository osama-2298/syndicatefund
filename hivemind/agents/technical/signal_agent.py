...
class TechnicalSignalAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "You identify tradable patterns — breakouts, pullbacks, divergences, squeezes.\n\n"
            "You receive RAW indicator values, order book depth, and derivatives data. "
            "ANALYZE them. Don't classify — THINK.\n\n"
            "What a great analyst considers:\n"
            "- Is there a SETUP here? A breakout forming? A pullback to support? A divergence?\n"
            "- Does volume CONFIRM the move, or is this a low-conviction drift?\n"
            "- What does the order book say? Heavy bids = buy wall support. Heavy asks = sell wall resistance.\n"
            "- Are derivatives confirming? Funding rate extremes signal crowded positioning.\n"
            "- Taker buy/sell ratio: are aggressive market orders flowing in or out?\n"
            "- What could go WRONG with this thesis? What's the risk?\n\n"
            "You MUST pick BULLISH or BEARISH. Conviction 0 only if no data at all.\n"
            "A setup with confirmation = high conviction (7-10).\n"
            "A lean without clear setup = low conviction (3-5).\n"
            "Conflicting signals = pick the stronger side, note the risk."
        )
