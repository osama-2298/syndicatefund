...
from hivemind.board.cpo_agent import CPOAgent
from hivemind.board.cso_agent import CSOAgent
from hivemind.board.cto_agent import CTOAgent
from hivemind.board.guardrails import validate_prompt
...
class BoardSession:
    ...
    async def convene(self, trigger: str, include_performance_review: bool = False) -> dict[str, Any]:
        ...
        # Query latest research findings for Board context
        research_context = {}
        try:
            from hivemind.db.models import ResearchReportRow
            from sqlalchemy import desc

            # Get latest signal health report
            result = await self.db.execute(
                select(ResearchReportRow)
                .where(ResearchReportRow.report_type == "signal_decay")
                .order_by(desc(ResearchReportRow.created_at))
                .limit(1)
            )
            signal_report = result.scalar_one_or_none()
            if signal_report:
                research_context["signal_health"] = signal_report.findings
                research_context["signal_recommendations"] = signal_report.recommendations

            # Get latest weekly digest
            result = await self.db.execute(
                select(ResearchReportRow)
                .where(ResearchReportRow.report_type == "weekly_digest")
                .order_by(desc(ResearchReportRow.created_at))
                .limit(1)
            )
            digest = result.scalar_one_or_none()
            if digest:
                research_context["weekly_digest"] = digest.findings
                research_context["research_recommendations"] = digest.recommendations
        except Exception as e:
            logger.warning("board_research_query_failed", error=str(e))
        ...
        if research_context:
            # CSO sees research findings
            cso_result["research_context"] = research_context
        ...
        summary["research_context"] = research_context
    ...
    async def _create_team(self, team_data: dict) -> None:
        ...
        from hivemind.board.guardrails import validate_data_keys
        from hivemind.data.data_layer import MarketSnapshot
